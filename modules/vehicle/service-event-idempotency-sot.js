...
(function(root){
 ...
 function key(input={}){if(input.idempotencyKey)return s(input.idempotencyKey);const v=s(input.vehicleId),c=s(input.serviceComponentId||input.categoryId),d=s(input.date),a=s(input.actionType||'default'),b=s(input.batchId);return v&&c&&d?`service:${v}:${c}:${d}:${a}:${b}`:null;}
