  ...
  const groupsHtml = groups.map((group) => {
    ...
    const rowsHtml = group.items.map(({ row, idx }) => {
      ...
      return '<div class="tx-item" style="align-items:flex-start">'
        + '<input type="checkbox" ' + checkedAttr + ' style="width:18px;height:18px;margin-top:8px;flex-shrink:0" data-onchange="VehicleCatalogImportUI.toggleRow" data-onchange-args=\'[' + idx + ']\'>'
        + '<div class="tx-info" style="flex:1">'
        + '<input type="text" class="fi" style="margin-bottom:6px" value="' + escapeHtml(row.partName || '') + '" placeholder="Nama part" data-oninput="VehicleCatalogImportUI.editField" data-oninput-args=\'[' + idx + ',"partName","$value"]\'>'
        + '<input type="text" class="fi" style="margin-bottom:6px" value="' + escapeHtml(row.category || '') + '" placeholder="Kategori (opsional, otomatis dari PDF)" data-oninput="VehicleCatalogImportUI.editField" data-oninput-args=\'[' + idx + ',"category","$value"]\'>'
        + '<div class="u-flex u-gap8">'
        + '<input type="text" class="fi" style="flex:1" value="' + escapeHtml(row.oemCode || '') + '" placeholder="OEM code (opsional)" data-oninput="VehicleCatalogImportUI.editField" data-oninput-args=\'[' + idx + ',"oemCode","$value"]\'>'
        + '<input type="number" class="fi" style="flex:1" value="' + priceVal + '" placeholder="Harga (opsional)" inputmode="numeric" data-oninput="VehicleCatalogImportUI.editField" data-oninput-args=\'[' + idx + ',"price","$value"]\'>'
        + '</div></div></div>';
    }).join('');
