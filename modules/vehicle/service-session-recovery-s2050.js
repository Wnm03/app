...
function prepare(ctx,payload){
 ...
 return{version:VERSION,state:'PREPARED',createdAt:new Date().toISOString(),sessionId:str(ctx.sessionId),vehicleId:str(ctx.vehicleId),rows:clone(rows),tx,stock,sparepartCats:clone(Array.isArray(g.D.sparepartCats)?g.D.sparepartCats:[]),stockMissing};
}
