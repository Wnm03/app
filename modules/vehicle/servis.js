...
syncServiceCostSummary(){const summary=typeof ServisChecklist!=='undefined'&&typeof ServisChecklist.costSummary==='function'?ServisChecklist.costSummary():null;const box=document.getElementById('servisChecklistCostSummary');if(box&&summary)box.innerHTML=`💰 <b>Total Rp ${Number(summary.total||0).toLocaleString('id-ID')}</b> · Jasa Rp ${Number(summary.labor||0).toLocaleString('id-ID')} · Part Rp ${Number(summary.parts||0).toLocaleString('id-ID')} · Bahan Rp ${Number(summary.consumables||0).toLocaleString('id-ID')} · Lain Rp ${Number(summary.other||0).toLocaleString('id-ID')}`;const costEl=document.getElementById('servisCost');const hasRows=typeof ServisChecklist!=='undefined'&&typeof ServisChecklist.toLogPayload==='function'&&ServisChecklist.toLogPayload().length>0;if(costEl&&summary&&hasRows){costEl.value=summary.total||0;costEl.readOnly=true;costEl.setAttribute('aria-readonly','true');}else if(costEl)costEl.readOnly=false;return summary;},
...
const cards=groups.map(found=>{
  ...
  const rows=groupItems.map((it,ii)=>{
    ...
    const componentCost=checked&&typeof ServisChecklist.getItemCost==='function'?ServisChecklist.getItemCost(it.id):null;
    const costInput=(field,label)=>`<label style="display:block;font-size:10px;color:var(--text2);margin-bottom:3px">${label}<input type="number" min="0" step="1" inputmode="decimal" class="fi" style="margin-top:3px;font-size:11px" value="${componentCost&&componentCost[field]!=null?escapeHtml(String(componentCost[field])):''}" placeholder="0" data-oninput="Servis.setServiceChecklistCost" data-oninput-args="${escapeHtml(JSON.stringify([gi,ii,field,'$value']))}"></label>`;
    const costHtml=checked?`<details style="margin-top:8px;background:var(--surface2);border:1px solid var(--border2);border-radius:10px;padding:0 9px"><summary style="cursor:pointer;padding:8px 0;font-size:11px;font-weight:700">💰 Biaya komponen${componentCost?` · Rp ${Number(componentCost.total||0).toLocaleString('id-ID')}`:''}</summary><div style="padding-bottom:8px;display:grid;grid-template-columns:1fr 1fr;gap:6px">${costInput('labor','Jasa')}${costInput('parts','Part')}${costInput('consumables','Bahan habis pakai')}${costInput('other','Lain')}</div><div class="u-fs10 u-t2">Kosong = belum diketahui · Rp0 = memang gratis/tidak ada biaya.</div></details>`:'';
...
const costSummary=typeof ServisChecklist.costSummary==='function'?ServisChecklist.costSummary():null;
box.innerHTML=`<div style="margin-bottom:8px"><div class="u-fw700 u-fs12">☑️ Checklist Komponen Servis</div><div class="u-fs11 u-t2">${groups.length} kategori aktif · ${checked}/${total} dikerjakan${na?` · ${na} tidak berlaku`:''}. Manual override tersedia.</div><div class="u-fs10 u-t2" style="margin-top:5px">Tindakan, hasil, catatan kondisi, identitas, dan biaya diatur langsung pada komponen yang dikerjakan.</div>${costSummary?`<div id="servisChecklistCostSummary" style="margin-top:8px;padding:9px 10px;background:var(--accent-soft);border:1px solid var(--accent);border-radius:10px;font-size:11px;line-height:1.5">💰 <b>Total Rp ${Number(costSummary.total||0).toLocaleString('id-ID')}</b> · Jasa Rp ${Number(costSummary.labor||0).toLocaleString('id-ID')} · Part Rp ${Number(costSummary.parts||0).toLocaleString('id-ID')} · Bahan Rp ${Number(costSummary.consumables||0).toLocaleString('id-ID')} · Lain Rp ${Number(costSummary.other||0).toLocaleString('id-ID')}</div>`:''}</div>${cards}`;
...
ensureServiceJobTypeUI(selectedJobType){
  ...
  if(sel){sel.innerHTML='<option value="">— Pilih bila relevan —</option>'+jobs.map(j=>`<option value="${escapeHtml(j.id)}">${escapeHtml(j.label)}</option>`).join('');sel.value=selectedJobType||'';} const compact=document.getElementById('servisJobTypeCompact');if(compact){compact.innerHTML='<option value="">— Pilih bila relevan —</option>'+jobs.map(j=>`<option value="${escapeHtml(j.id)}">${escapeHtml(j.label)}</option>`).join('');compact.value=selectedJobType||'';}
},
...
const _preSaveChecklistSeed=(typeof ServisChecklist!=='undefined'&&typeof ServisChecklist.toLogPayload==='function')?ServisChecklist.toLogPayload():[];
