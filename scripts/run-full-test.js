...
const detectedParallelism=Math.max(1,typeof os.availableParallelism==='function'?os.availableParallelism():os.cpus().length||1);
const defaultShardCount=detectedParallelism<=1?1:32;
const shardCount=Math.max(1,Math.min(Number(process.env.TEST_SHARDS)||defaultShardCount,64));
...
const defaultConcurrency=Math.max(1,Math.min(4,detectedParallelism));
