...
const car=read('car-notes.js'), tx=read('modules/finance/transaksi-b.js'), del=read('modules/finance/tx-list-cashflow.js'), bridge=read('modules/finance/tx-servis.js'), life=read('modules/vehicle/service-event-lifecycle.js');
...
test('finance-created service emits create/update lifecycle',()=>{assert(bridge.includes("ServiceEventLifecycle.update(s,{source:'finance'})"));assert(bridge.includes("ServiceEventLifecycle.create(log,{source:'finance'})"));});
