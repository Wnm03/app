...
test('checklist SoT is the current cumulative canonical contract after cumulative patching', () => {
  assert.equal(GROUPS.length, 13);
  assert.equal(ITEMS.length, 102);
  assert.equal(new Set(GROUPS.map(g => g.masterCategoryId)).size, 13);
});

test('current cumulative checklist preserves the explicitly accumulated 2026-09-12 components', () => {
