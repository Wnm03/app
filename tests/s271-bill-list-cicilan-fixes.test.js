...
test('openBillModal() — bill kind:"cicilan" aktif TANPA riwayat transaksi sama sekali -> toast peringatan, TIDAK panggil editTx()', () => {
  const calls = { editTx: [], toast: [] };
  const D = {
  ...
  assert.equal(calls.editTx.length, 0);
...
test('source guard: transaksi.js membuat/mengedit bill kind cicilan shared HARUS set totalAmount:perBulan, TIDAK BOLEH lagi totalAmount:total (regresi S271)', () => {
  ...
  assert.equal(goodPatternCount, 2, 'kedua cabang (edit existingBill & bill baru) harus pakai totalAmount:cicilanShared?perBulan:null');
