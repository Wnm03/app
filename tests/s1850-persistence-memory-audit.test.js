...
ok(s.includes('if(_saveQueuedVersion===version)return _saveQueuedStamp;'),'S1850: duplicate persistence queue guard missing');
ok(s.includes('var _saveQueuedStamp=0;'),'S1850: queued persistence stamp missing');
ok(s.includes("const _savePersistMetaKey='kw_v4_persist_meta';"),'S1850: durable persistence metadata missing');
