...
test('modal-write.js memakai satu loader batch untuk seluruh MODAL_HTML 0..102 di index.html', () => {
  const matches = [...INDEX_HTML.matchAll(/data-modal-range=\"(\d+)-(\d+)\"/g)].map((m) => [Number(m[1]), Number(m[2])]);
  assert.deepEqual(matches, [[0, 102]], 'index.html harus memakai tepat satu modal-write.js batch 0-102');
  assert.equal((stripHtmlComments(INDEX_HTML).match(/<script\s+src=\"modules\/shared\/modal-write\.js\?v=\d+\"[^>]*>/g) || []).length, 1);
});
