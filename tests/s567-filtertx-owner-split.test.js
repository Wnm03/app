...
test('showFilteredTx(scope=account) — akun tertaut aset multi-owner -> tab per pemilik (bukan patungan/gabung), detail default owner pertama', () => {
  ...
  // Sesi (audit user "Porsi per Pemilik bukan sistem patungan"): transaksi t1/t2
  // TIDAK punya ownerPorsiId eksplisit (belum ada field ini saat itu ditulis) --
  // jadi keduanya fallback ke owner PERTAMA (renov) secara PENUH (bukan proporsi
  // % lagi). Modal (income) 1.000.000 & Pengeluaran 200.000 semuanya masuk ke
  // renov, owner kedua (mas sihab) dapat 0.
  assert.ok(html.includes('Modal Rp1000000'), 'transaksi tanpa ownerPorsiId fallback penuh ke owner pertama (renov)');
  assert.ok(html.includes('Pengeluaran Rp200000'), 'pengeluaran fallback penuh ke owner pertama (renov)');
  assert.ok(html.includes('Total Rp800000'), 'total net fallback penuh ke owner pertama (renov)');
  ...
  assert.ok(rows[1].detailHtml.includes('Modal Rp0'), 'owner kedua tidak dapat bagian krn tidak ada tx yang ditandai utk dia');
});
...
test('showFilteredTx(scope=account) — transaksi dgn ownerPorsiId eksplisit -> dihitung ke owner yang DIPILIH, bukan owner pertama/proporsi', () => {
  ...
  assert.ok(rows[0].detailHtml.includes('Pengeluaran Rp200000'), 't2 tanpa ownerPorsiId tetap fallback ke owner pertama (renov)');
