- 2026-09-14 (v1702, merge dari 2 sesi paralel di atas baseline v1700): user mengupload 2 ZIP patch terpisah yang SAMA-SAMA dibangun dari baseline v1700 tapi tidak pernah saling ketemu — PATCH-sesi-servis-actiontype-v3 (fitur "Tindakan Servis" periksa/bersih/ganti: `Servis.syncServiceActionType()`, `resolveLogServiceComponentId()` resolver SOT tunggal komponen servis, `openHistoryFromReminder()`, dropdown `servisActionTypeWrap` di modal Riwayat Servis) vs PATCH-v1701-riwayat-servis-kategori-komponen-bocor-antar-record (fix bug dropdown Kategori/Komponen Servis "bocor" antar-record: `catEl.value=''`/`compEl.value=''` di-reset SEBELUM populate, di `Servis.renderServiceInputSelectors()` car-notes.js & `renderTxServisSelectors()` tx-servis.js). Merge langsung ke ZIP v1701 saja akan DIAM-DIAM menghilangkan seluruh fitur actionType — audit dulu, dikonfirmasi user, baru merge manual (bukan overwrite): baseline v3 (actionType) jadi dasar `car-notes.js`, fix reset v1701 disisipkan tepat sebelum `ServiceInputCatalog.populateCategorySelect()` di dalam `renderServiceInputSelectors()` yang sudah bertambah param `selectedActionType` — 0 baris v3 lain diubah/dihapus; `tx-servis.js` diambil langsung dari v1701 (v3 tidak menyentuh file ini, 0 konflik); `modals.js` dasar baseline + markup `servisActionTypeWrap` dari v3 disisipkan setelah select `servisComponent`. 2 test file dari masing-masing sesi digabung apa adanya; 1 test lama (`service-filter-s17.test.js`) & 1 test dari v3 (`servis-riwayat-category-stale-leak-fix.test.js`) butuh penyesuaian assersi murni (tanda tangan fungsi jadi 3 parameter & filter komponen kini lewat `resolveLogServiceComponentId()` alih-alih baca `s.checklist` langsung — SUDAH diverifikasi bahwa perubahan filter ini memang bagian dari perbaikan v3, bukan regresi), 0 logic produksi tambahan diubah untuk lolos test. Full suite 6737/6737 pass, 0 fail. `node scripts/build.js` (checkout penuh, dijalankan langsung) sukses — versi disinkron ke `sesi-servis-actiontype-riwayat-leak-merge-1702`/`?v=1702`/`kw-cache-v1702`, bundle a/b ditulis ulang (tanpa minifikasi, esbuild tidak ada di sandbox, sintaks lolos `node --check`), `FILE-MAP.md`/`COVERAGE-PER-MODULE.md` diregenerasi. ZIP PATCH kumulatif (bukan full release) diserahkan — hanya file yang berubah dari baseline v1700 (14 file + 2 test baru).
- 2026-09-13: Sesi perbaikan 3 failure terakhir (follow-up dari PATCH-AKUMULASI-S02-S31-S41-S42-FINAL-29FIX-OVERLAY-VERIFIED, lihat SESSION-NOTE-sesi-fix-29-terakhir-3-fail.md). (1) `servis-edit-reminder-tab-sot.test.js`: `modules/shared/modals.js` (PRODUCTION) mendapat markup tab Detail/Pengingat (`servisEditTabs`/`servisDetailPanel`/`servisReminderPanel`) yang sebelumnya belum ada walau sisi JS (`Servis.setEditTab()`/`renderEditReminderTab()`, Sesi 4A) sudah lengkap; test-nya juga dibetulkan dari path orphan `modules/modals.js` ke SoT `modules/shared/modals.js`. (2) `servis-mastercategoryfilter-uncategorized-persist-sesi-d-lanjutan5.test.js`: test-contract only -- assersi label diupdate ke "❔ Belum dikategorikan" (mengikuti UX dropdown yang sudah didokumentasikan sebelumnya) & mock `insertAdjacentElement` dibuat routing by-id (robust thd row filter baru `renderServiceComponentFilter`). (3) `virtual-bill-manual-scenario-s468d.test.js`: test-contract only -- assersi routing tap kartu tagihan virtual diupdate dari `openBillModal` ke `markBillPaid` (UX yang sudah didokumentasikan sebelumnya). Full suite: 6596/6596 pass, 0 fail, 0 regresi (sebelumnya 6593/6596, 3 fail identik dgn yang dilaporkan user). ZIP PATCH akumulasi (bukan full release) diserahkan; 60 file dari patch sebelumnya dipertahankan apa adanya.
- 2026-09-12 (v1685, delta zip di atas v1684): Sesi ringan lanjutan dari catatan C di luar scope fix pokok-prorata itu sendiri — 2 item dikerjakan (item lain: konfirmasi aturan bisnis prorata-turun masih menunggu jawaban C, belum diasumsikan; item test lain di luar domain payroll — regex boot-pin-idempotent, hardcode path service-maintenance, gate service-sot-integrity — TIDAK disentuh sesi ini krn file terkait tidak ada di delta yang diupload, butuh upload terpisah). (1) `Payroll.jamKurang(w)` baru (`modules/business/payroll-absensi.js`) + dipakai `renderWorkDays()` -- Riwayat Absensi sekarang tampil `(kurang N jam)` kalau `totalJam<7` (jenis biasa saja, minggu/borongan tidak relevan), melengkapi `(lembur N jam)` yang sudah ada, biar user langsung paham kenapa nominal beda. (2) Migrasi data lama: `Payroll.auditPokokProrataLama()` mendeteksi entri `D.workDays` yang tersimpan SEBELUM fix v1684 (masih `pokok` flat = `gajiHariInput` penuh walau `totalJam<7`); `fixPokokProrataLama()` mengoreksi `pokok` & `total`-nya (mempertahankan tambahan/potongan); `runPokokProrataLamaMigration()` + `renderPokokProrataLamaBox()` menampilkan box peringatan & tombol koreksi 1-klik di modal Absensi (`whPokokProrataLamaBox`, markup baru di `modules/shared/modals.js`), dgn konfirmasi user dulu sebelum menulis. Aman dijalankan berulang (idempotent) & tidak menyentuh entri jenis minggu/borongan atau entri yang sudah benar. Test baru `tests/payroll-absensi-kurang-jam-label-dan-migrasi-lama.test.js` (13 test) + verifikasi manual via sandbox VM langsung terhadap kode sebenarnya (bukan cuma logika terisolasi) utk jamKurang/audit/fix/migration-flow, semua sesuai ekspektasi. Full suite belum dijalankan sesi ini (delta zip parsial, file test lain di luar domain payroll tidak diupload C) — perlu checkout penuh + `node scripts/build.js` + `npm test` di device C sebelum rilis; ZIP PATCH (bukan full release) diserahkan.
